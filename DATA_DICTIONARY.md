...
Tiers: `core` ships and is the default table; `extended` ships (style, availability, price/own, derived rates); `archive` is remaining Opta fields and `snap_player_day` extras — mart only, never metric values in `web/data`. `web/data/metrics_register.json` is the same CSV.
