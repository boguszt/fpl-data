...
Every Opta id is prefixed `o_` (`o_total_pass`, `o_total_tackle`, …) so FPL keys (`tackles`, `goals`, …) cannot collide at the data layer. Shipped Opta season counts land on `players_{season}.json` as `{id}_total`; derived ratios (`o_pass_accuracy`, …) land as `{id}` already computed, NULL when the denominator is 0. Nothing Opta goes into `matchlogs_{season}.json`.
...
Shipped core/extended Opta fields are exported on `players_{season}.json` with an `o_` prefix (`o_total_pass_total`, `o_pass_accuracy`, …). Archive Opta columns stay mart-only. Nothing Opta is written to match logs.
...
Web export: `web/data/clusters.json` (feature order, labels, centroid z-vector per cluster) and `web/data/style_{season}.json` (only seasons with assignments). Each player is `{z, c, d, c2, d2}` — `z` is the clustering z-vector in the order declared in `clusters.json.features`, for live cosine similarity in the browser.
